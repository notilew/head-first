...
for (let i = 0; i < crazyArray.length; i++) {
    const typeOfElement = typeof crazyArray[i];
    const isElementABoolean = typeOfElement === 'boolean';
    const isElementANumber = typeOfElement === 'number'

    if (isElementABoolean) {
        booleanAmout++;
    } else if (isElementANumber) {
        numberAmout++;
    } else {
        stringAmout++;
    }
...
console.log(`O crazyArray tem ${booleanAmout} booleans, ${numberAmout} numbers e ${stringAmout} strings.`);
...
console.log(`Números Pares: ${evenNumbersString}. Números Ímpares: ${oddNumbersString}`);