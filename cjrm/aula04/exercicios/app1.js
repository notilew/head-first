...
let numberToCheck = 337;
let sumNumbersOfArray = 0;

for (let i = 0; i < numbers.length; i++) {
    sumNumbersOfArray += Number(numbers[i]);
}

console.log(`O total da soma é ${numberToCheck}: ${sumNumbersOfArray === numberToCheck}`);
...
for (let i = 0; i < arrays.length; i++) {
    for (let j = 0; j < arrays[i].length; j++) {
        sumNumbersOfArrays += arrays[i][j];
    }
}

console.log(sumNumbersOfArrays);
