...
console.log(convertToString(1550));
...
console.log(numberOfCharacters('wellington'));
...
console.log(indexOfString('g', 'wellington'));
...
const concatArray = (array01 = [], array02 = []) => array01.concat(array02);
...
const showName = () => console.log('wellington felix');
...
const myFunc2 = (value = 0, callback) => callback(value);
const tripleOfNumber = (value = 0) => value ** 3;

console.log(myFunc2(33, tripleOfNumber));
...
numbers.forEach((number, index, array) => console.log(`O ${++index}º item do array ${array} é ${number}.`));
...
const amountOfLiked = (people = []) => {
    let message = '';
    const lastPersonIndex = people.length;
    const lastPerson = people[lastPersonIndex - 1];
    ...
    switch (people.length) {
        ...
        case 1:
            message = people.forEach((person) => `${person} curtiu isso!`);
            break;

        case 2:
            message = `${people.join(' e ')} cutiram isso!`;
            break;

        case 3:
            message = `${people.join(', ').replace(`, ${lastPerson}`, ` e ${lastPerson}`)}`;
            break;
        ...
        default:
            ...
            message += `e mais ${people.length - 2} pessoas curtiram isso!`;
...
console.log(amountOfLiked(['wellington', 'palloma', 'jose', 'maria']));